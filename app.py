...
def run_query_streaming_to_excel(query, report_name, database_name):
    conn = get_connection(database_name)
    cursor = conn.cursor()
...
# optional quick list – add your common DBs here
predefined_dbs = ["OnlineRetailDb", "master", "tempdb"]
selected_from_list = st.selectbox(
    "Choose a database (or select 'Custom' to type manually)",
    options=predefined_dbs + ["Custom"],
    index=0
)

if selected_from_list == "Custom":
    database_name = st.text_input("Enter database name", value="OnlineRetailDb")
else:
    database_name = selected_from_list

st.caption(f"Current database: **{database_name}**")
...
if uploaded_file:
    ...
    if st.button("Run and Generate Report"):
        if not database_name:
            st.error("Please provide a database name before running the query.")
        else:
            try:
                if use_streaming:
                    excel_path = run_query_streaming_to_excel(query_text, report_name, database_name)
                    st.success(f"Large query streamed and Excel report generated from DB '{database_name}'!")
                else:
                    df = run_query(query_text, database_name)
                    st.success(f"Query executed successfully on DB '{database_name}'!")
...
Automation Bot"""
                ...
                st.error(f"Error while running on database '{database_name}': {e}")
